...
![Mapa de capacidades de Tere: conversación nativa por texto, voz y fotos; conocimiento verificado con consultas en vivo a la base de productos; operación comercial con cotizaciones y registro de leads en el CRM; y operada como un producto con configuración versionada](/writing/tere/capability-map-ES.png)
...
![Diagrama de arquitectura de Tere: los mensajes de WhatsApp — texto, voz e imagen — pasan por la capa de canales hacia el runtime del agente con sus cuatro capas de comportamiento versionadas y un ciclo ReAct con herramientas conectadas a la base de productos en vivo y al CRM](/writing/tere/architecture-ES.png)
...
*Una foto de tela son en realidad dos problemas de visión.* El reconocimiento de telas es parte de la configuración de Tere, y lo primero que decide su análisis de imagen es qué está viendo. Una foto real de una muestra se analiza por sus características textiles — textura, tejido, fibra — mientras que stickers, memes y fotos de personas se marcan como "no tela" y se ignoran en silencio. Una *textura* es una pregunta de catálogo: "piel de elefante", un tacto aterciopelado o un tejido acanalado se convierten en búsquedas en la base de datos. Y la regla que hace esto seguro en producción: la visión solo propone candidatas. Antes de afirmar nada, Tere las confirma contra el catálogo en vivo y responde con nombres reales de tela, máximo tres candidatas. Nunca declara una coincidencia que la base de datos no pueda respaldar.

*Los estampados son el segundo problema, y ahí la búsqueda genérica de imágenes falla.* La foto de un *estampado* significa que el cliente quiere el diseño exacto de un catálogo de unas 10.000 imágenes de sublimación. Nuestro primer pipeline era el estándar: un modelo de visión describe la foto, convertimos la descripción en un embedding y buscamos. No servía para coincidencia visual exacta, porque las descripciones no son deterministas y descartan el detalle a nivel de píxel — la foto de consulta devolvía diseños parecidos en tema, no el diseño en sí. Lo reconstruimos sobre embeddings multimodales: las imágenes y las consultas de texto comparten un mismo espacio vectorial, una sola columna de embeddings atiende los dos modos de búsqueda sobre pgvector, y la misma foto ahora encuentra su diseño del catálogo casi a la perfección. El modelo de visión sigue ahí, pero solo para auto-etiquetar metadatos; ya no está en la ruta de búsqueda. La misma lección que con los precios: cuando la respuesta tiene que ser exacta, la recuperación tiene que estar anclada en el activo real, no en la paráfrasis que hace un modelo.
